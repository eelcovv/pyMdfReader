...
class MDHFileHeader(MDFBlock):
    ...
    Parameters
    ----------
    mdf_stream : file object
        reference to mdf file

    Attributes
    ----------
    version : str
        string with the version. such as 2.1
    version_minor : UINT16
        With minor version (1)
    version_major : UINT16
        With major version (2)
    status_record_position : LONG
        Refers to status of type 10. Must be 0 if unused
    created_by : LONG
        Generated by: 0=User, 1=MLab
    mdf_header_size : LONG
        size of the header (normally 72)
    store_type : LONG
        storage method.
        0. Multiplexed
        1. Blockwised (currently unused)
    file_type : LONG
        File type
    frame_size : LONG
        Size of the data frame in bytes
    no_of_data_sets : LONG
...
class DataSetRecord(MDFBlock):
    ...
    # define the data formats inside this class
